...
int binary_search(int find, int start, int end, int array[])
{
	...
	if (start <= end) {
		if (array[mid] == find) {
			printf("%d is find, position is %d!\n", find, mid);
			return 1;
		}
	
		if (array[mid] > find) {
			result = binary_search(find,start,mid-1,array);
		}
		
		if (array[mid] < find) {
			result = binary_search(find,mid+1,end,array);
		}
	} else if (result == 0) {
...
int main(int argc, char **argv)
{
	...
	binary_search(input,0,N-1,rand_data);
